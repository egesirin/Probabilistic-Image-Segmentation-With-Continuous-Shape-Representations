...
def train_diagonal(t, number_pre_epochs, mean, log_diagonal, loss_function, optimizer_m, optimizer_d_m,
    ...
    if t <= number_pre_epochs:
        ...
        mc_samples = mc_sample_mean(mean_vec, dim, number_of_samples)
    else:
        optimizer = optimizer_d_m
        mean_vec, diagonal_vec = from_funct_to_matrix_cov_is_diagonal(mean, log_diagonal, dim)
        mc_samples = mc_sample_cov_is_diagonal(mean_vec, diagonal_vec, number_of_samples)

    for j in range(number_of_samples):
        log_prob[0][j] = -loss_function(mc_samples[j], gts[0])
        log_prob[1][j] = -loss_function(mc_samples[j], gts[1])
