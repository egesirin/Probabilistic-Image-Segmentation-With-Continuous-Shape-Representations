...
def main():
    ...
    for t in range(epoch):
        train(t, models, model_type, criterion, optimizers, number_of_mc_sample, coordinates, train_dataloader,
              batch_size, DEVICE, writer)
        validation(t, models, model_type, criterion, number_of_mc_sample, number_of_val_sample, coordinates,
                   validation_dataloader, batch_size, DEVICE, writer, train_dataloader)

    writer.close()
    model_path = "mean.pt"
    torch.save({
        'mean_function_state_dict': models.state_dict(),
